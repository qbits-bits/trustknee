...
SEED = 42
ID_COLS = [
    "window_index", "start_time_s", "end_time_s", "subject_id",
    "trial_num", "label_id", "exercise", "execution", "label",
]
ENG_PATTERNS = [
    "sym_ratio", "ctrl_ratio", "loadshare", "lrshare", "ctx_",
    "_norm", "_delta", "_absd", "_lag", "_ema", "_rmin", "_rmax",
    "_rrange", "_rm", "_rs", "inter_", "_symdelta",
]
MODEL_CONFIGS = {
    "deep_slow": dict(
        n_estimators=1200, max_depth=6, learning_rate=.02, subsample=.85,
        colsample_bytree=.85, min_child_weight=3, reg_lambda=2.0, reg_alpha=.1,
    ),
    "wide_reg": dict(
        n_estimators=1000, max_depth=7, learning_rate=.03, subsample=.8,
        colsample_bytree=.8, min_child_weight=5, reg_lambda=3.0, reg_alpha=.2,
    ),
    "shallow": dict(
        n_estimators=1500, max_depth=5, learning_rate=.02, subsample=.9,
...
def build_features(df):
    ...
    anchors = {
        s: {t: _sensor_col(df, s, t) for t in ["emg", "gyro", "acc"]}
        for s in range(1, 9)
    }
    ...
    subj_med = (
        df.groupby("subject_id")[anchor_cols]
        .expanding()
        .median()
    ...
    for s in range(1, 9):
        for t in ["emg", "gyro", "acc"]:
            ...
            if c and c in df.columns:
                ...
                df[c + "_delta1"] = d1
                df[c + "_delta2"] = d1.groupby(
                    [df["subject_id"], df["trial_num"]]
                ).diff().fillna(0)
                df[c + "_absd1"] = d1.abs()
    ...
    def rmean(c, w):
        return grp[c].transform(
            lambda x: x.rolling(w, min_periods=1, center=True).mean()
        )

    def rstd(c, w):
        return grp[c].transform(
            lambda x: x.rolling(w, min_periods=1, center=True).std().fillna(0)
        )

    def rmin(c, w):
        return grp[c].transform(
            lambda x: x.rolling(w, min_periods=1, center=True).min()
        )

    def rmax(c, w):
        return grp[c].transform(
            lambda x: x.rolling(w, min_periods=1, center=True).max()
        )
    ...
    feat_cols = [
        ...
        if c not in ID_COLS and any(p in c for p in ENG_PATTERNS)
    ]
    return df, feat_cols
...
def fit_pipeline(tr, va, te, feat_cols):
    ...
    rk = xgb.XGBClassifier(
        n_estimators=200, max_depth=5, learning_rate=.1, tree_method="hist",
    ...
    for K in [k for k in [220, 270, 320, len(feat_cols)] if k <= len(feat_cols)]:
        ...
        q = xgb.XGBClassifier(
            n_estimators=400, max_depth=6, learning_rate=.05, subsample=.8,
            colsample_bytree=.8, tree_method="hist", n_jobs=-1,
...
def main():
    ...
    ap.add_argument("--legacy", action="store_true",
                    help="use original Phase-2 seeded 70/15/15 split")
    args = ap.parse_args()
    out_dir = args.out or (
        "models/xgboost_phase2split" if args.legacy
        else "models/xgboost_canonical"
    )
